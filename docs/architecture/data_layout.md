...
├── raw/                 # Immutable source material and acquisition provenance
├── staging/             # Parsed or cleaned data that has not passed publication checks
...
- `raw`: append-only; acquisition code may add files but never rewrite history.
...
- `.runtime`: browser profiles and logs; it is not research data.
...
- City HPI remains city-level and must be joined late; it should not be
  interpreted as independent grid-level information.
