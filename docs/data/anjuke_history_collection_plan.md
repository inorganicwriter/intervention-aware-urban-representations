...
- **解析/入库管线**：`build_anjuke_label.py`（截面版）、`build_wayback_label.py`、
  `build_housing_price_panel.py` 已存在，新数据走同一纳入检查流程
...
| **预算上限确认** | 阶段 0 止损线 300 元；阶段 1 总预算需批准 | 无 |
...
且新增小区月观测 ≥ 500 万、交叉验证中位价差 < 15%。**
未达标城市写入论文数据支持边界，保持城市数据单独记录。
