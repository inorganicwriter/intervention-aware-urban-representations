...
状态：已冻结实现合同；正式 5,048 单元队列须在全部门禁通过后启动  
日期：2026-07-23

## 1. 目标

本项目的最终产品不是一个总体 DID 系数，而是处理网格 `i`、结果族 `k`、事件期 `h`
的局部因果响应标签：
...
- 开通当月是部分暴露月，主分析中排除，不作为处理前或处理后观测。
...
- 年度 anticipation：月度 6 个月窗口无法在年粒度面板中表达。年度 GSC 主规格
  使用 `annual_anticipation_years = 0`（不排除开通前一年，对应月度 0 个月敏感性
...
控制组选择只能读取处理发生前可观测的信息。以下内容禁止参与 donor admissibility、距离、
共同支持或阈值校准：处理后结果值、处理后结果是否缺失、处理后显著性和最终标签。
...
RMS 标准化差异和最大绝对标准化差异的第 95 百分位作为门槛。目标对三项均不超过门槛才
进入 matched-label 路径，否则路由到 GSC。门槛校准本身也不得读取任何处理后信息。
...
其中 `B` 是预先冻结的处理前基期或处理前均值。匹配设计遵循 Abadie–Imbens 最近邻匹配
的共同支持、允许放回和 Mahalanobis 距离规范。完整 cohort ATT 的偏差修正和解析方差由
R `Matching::Match` 单独报告；不得把测试版距离函数冒充完整估计器。
...
正式实现使用 R `gsynth`，two-way effects、处理前交叉验证选择因子数、`r=0:5`、
`min.T0=5`、200 次 parametric bootstrap。donor 身份只能由处理前支持决定；处理后缺失可使
某一期标签缺失或使模型失败，但不能触发基于处理后信息的 donor 重选。
...
接近零或统计不显著的标签仍然是有效标签，不得因显著性而删除。只在输入支持不足、
共同支持失败、预拟合不合格、估计器失败或目标期缺失时设置缺失掩码。
...
不得重复追加或改变结果。生产队列只有在合成真值、处理后泄漏、事件时间、真实数据、失败
