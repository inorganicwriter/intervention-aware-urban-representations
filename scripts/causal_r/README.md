...
Four-GPU production entry point (run in an isolated server working copy; omit
`--reset-queues` for the frozen project baseline):
...
Python production stays stopped until the shadow queue has run with
...
event-study workflow are Python-only; R remains an explicit audit and reference
backend for the qualification stage.

The production estimators are specified in
`docs/research/decisions/DDR-003_complete_published_estimators.md`.
Each estimator has its own formula, output and inference record. The project
route selects the estimator for each task.
...
The immutable treatment list contains 5,048 station grids. The following setup
commands rewrite queue/input files. Run them in an isolated server working copy;
the frozen local checkout remains read-only:
...
The production runner uses the full same-city donor pool. A benchmark with
16,514 donors exceeded ten minutes on the current machine. The
`test_real_panelmatch_gate.R` 300-donor fixture is an integration test and
stays outside formal estimates.
...
The runner stops when the treated cohort is too small to identify the
bias-adjustment regression and records the reason.
...
and holdout-path gates. A failed gate routes the task to GSC and records the
...
`asinh` transform. GSC donor selection uses complete clean
...
eight cores, while the 200-replication bootstrap is sequential to limit memory
use. A real-data smoke test is explicitly requested and uses an output directory
...
it records metadata while jackknife remains the inference method.
...
masked with a pre-treatment-only value before lambda construction, CV, and
  ...
  explicit `mc_regularized=FALSE` marker.
...
path and has no single physical `control_grid_id`.
...
44-city, 2012-01 to 2024-12 monthly cache before the first grid is processed:
...
pair stops the current production run and records a cache-contract failure.
...
Remove `--dry-run` only for a bounded test. The control-design queue records a
...
batches (for example 25-50 rows) so persistent per-grid records are synchronized
...
tasks (`5,048 × 4` outcome families). One production GSC population bounded test
for order 906 is terminal. The larger task count records outcome families for
the same treatment units. Remove `--dry-run` after the full gate suite passes.
Monthly VIIRS tasks
...
The first command checks queue completion, smoke products, key consistency,
...
`--allow-partial` is available on both commands for bounded tests and unit-test
fixtures. Partial products carry a non-production marker and remain outside
formal training.
...
diagnostic metadata. Sample selection follows the formal design and Response
Artifact checks.

Only outputs with production manifests (`run_mode=production`,
`production_eligible=TRUE`) enter the formal release. Smoke and bounded-test
artifacts remain outside that release.
...
- `event_study_report.md`: reading guide. Near-zero pre-period means support the
  current specification; the joint test is read together with the design-stage
  placebo evidence;
